...
#' @description provides mean score estimates of parameters for GEE model
#' of response variable using different covariance structure
#' @details It uses the mean score method to reduce the bias
#' due to missing covariate in GEE model.The response variable \eqn{\mathbf{Y}} is related to the coariates as \eqn{g(\mu)=\mathbf{X}\beta}, where \code{g} is the link function for the glm. The estimating equation is
#' \deqn{\sum_{i=1}^{k}\sum_{j=1}^{n}(\delta_{ij}S(Y_{ij},\mathbf{X}_{ij},\mathbf{X}'_{ij})+(1-\delta_{ij})\phi(\mathbf{V}=\mathbf{v}))=0}
...
#'
MeanScore<-function(data,formula,id,visit,family,init.beta=NULL,init.alpha=NULL,
