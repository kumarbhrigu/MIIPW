#' miSIPW
#' @title Fit a geeglm model using miSIPW
...
#' @param weights A vector of weights for each observation. If an observation has weight 0, it is excluded from the calculations of any parameters. Observations with a NA anywhere (even in variables not included in the model) will be assigned a weight of 0. Weights are updated as the mentioned the details.
...
#' @param pMat pMat predictor matrix as obtained in \code{\link[mice]{mice}}
...
#' @seealso   \link[MIIPW]{SIPW},\link[MIIPW]{AIPW},\link[MIIPW]{miAIPW}
miSIPW<-function(data,formula,id,visit,family,init.beta=NULL,init.alpha=NULL,
